...
<template>
  <div>
    <div class="text-center">
      <h2 class="text-5xl text-red-600 font-semibold">{{ stepForm.device.brand.name }} {{ stepForm.device.name }} Repair
      </h2>
    </div>
    <div class="my-6 flex gap-8 flex-wrap justify-center max-w-2xl mx-auto">
      <label :for="repair.repair_type"
        ...
        <input class="hidden" type="radio" v-model="stepForm.form.repair_type" :value="repair.repair_type"
          name="repair_type" :id="repair.repair_type">
      </label>
    ...
    <div class="border-red-500 border-2 py-8 px-6 max-w-4xl rounded-md mx-auto">
      <div class="flex flex-col sm:flex-row text-center">
        <div class="w-full space-y-4 px-2 py-8"
          :class="(index !== selectedRepair.length - 1) ? 'border-red-600 border-b-2 sm:border-r-2 sm:border-b-0' : ''"
          v-for="(repair, index) in selectedRepair" :key="index">
          <h3 class="text-red-600 font-bold text-4xl">
            {{ repair.title }}
          </h3>
          <p>
            Lorem ipsum dolor, sit amet consectetur adipisicing elit. Atque quaerat, id optio dolor culpa maxime, expedita
            rerum placeat ipsa deserunt inventore modi. Ducimus fugiat culpa maiores officiis laborum ipsa veniam.
          </p>
          <h2 class="text-3xl text-gray-800 font-bold">
            {{ repair.price }}

          </h2>
          <button @click.prevent="selectOption(repair.id)"
            class="btn-action px-9 py-1 bg-red-600 text-white font-bold text-xl before:bg-black">Book</button>
        </div>
